#!/usr/bin/env python3
"""Figure S1 (level-space companion to the slope montage S2): per-population ln of the ESP-2013 age-
standardised death rate, indexed to the fitted 2019 level, on a COMMON vertical axis. Baselines from the
two-step model, projected 2019->2025:
  TTa   (own two-step β,γ)         — red dashed projection
  STTa  (precision-shrunk β,γ)     — teal: solid over the fit years, dashed 2019->2025
  STTa+ (slope-anchored)           — purple solid; the shrunk trend refitted after adding the observed
                                     2019->2024/25 return slope to the moving-slope regression.
The vertical gap between observed (black) and a baseline over 2020-2025 is that model's excess; 2020-2023 shaded.
Here STTa+ reads as a CURVE bending onto 2024-25 (in the slope figure S2 the same curve is a straight tilt).
Writes output/fig_option1_logquad_montage_STTaplus.png ."""
...
for l in locs:
    ...
    D[l]=(start,obs_y,oi,xf,tta,stta,sttap)
...
for k,l in enumerate(locs):
    ax=axes[k]; start,obs_y,oi,xf,tta,stta,sttap=D[l]; fitmask=xf<=2019
...
hp=[plt.Line2D([0],[0],color=COBS,lw=2.4,label="Observed ln(rate)"),
    plt.Line2D([0],[0],color=CTT,lw=2.4,ls=(0,(3,2)),label="TTa"),
    plt.Line2D([0],[0],color=CST,lw=2.4,label="STTa"),
    plt.Line2D([0],[0],color=CSP,lw=2.4,label="STTa+"),
...
FIG=os.path.join(OUTD,"fig_option1_logquad_montage_STTaplus.png"); fig.savefig(FIG,dpi=140,bbox_inches="tight"); plt.close(fig)
